...
4. **失败即停**：命令失败、超时、权限不足、sandbox / network 错误、CI pending 或输出异常时，先停止并说明错误类别与建议下一步；不得连续重试或继续串行执行后续步骤。sandbox / 权限受限环境内发生进程创建类错误（spawn 失败、Win32「找不到文件」等，无断言失败输出）时，按环境故障分流：换非沙箱环境复核一次再定性，不得记为脚本或产物缺陷，也不得以其阻塞登记 / 收尾类工作（口径见 `ai/rules-core.md` §2）。
...
**同步覆盖件保护**：派生仓中的 `CONTRIBUTING.md` / `ai/prompts/maintainers/*` 是同步覆盖件；派生仓会话中若拟修改这些文件，写入确认时必须附带「此为同步覆盖件，直改会被下次同步覆盖，应走回流提案」提示。`MAINTAINERS.md` 为模板维护者手册，v1.70.0 起不下行、不在同步清单；存量派生仓中的该文件属历史残留，随 post-sync-cleanup 审计清理，不应继续修改。
