...
| 用例本体表（可选增强） | 用例 ID、用例名、参与者、前置 / 后置条件、主流程摘要、关联 SC-ID / U-ID / REQ-ID；作为 `DIAG-UC-NN` 用例图的结构化底表与追溯锚点（转换映射见 `ai/document-lifecycle-rules.md` §2.2）；与用例图同为 OO overlay 可选项，不设为默认必备 |
