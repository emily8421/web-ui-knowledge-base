...
若触发 UI 原型策略，必须引用 `docs/05-tech-spec.md`、`ai/project-rules.md` §2.3 或 `docs/design/frontend-interaction.md` 中的原型策略记录，并说明原型位置、覆盖页面 / 主流程 / 状态 / 设备范围、未覆盖项和确认状态。还应说明默认 UI 标准基线（如后台 / 知识库工作台 / 数据密集表格 / 问答界面 / 营销页）、信息密度、字号 / 行高倾向、是否偏离默认行业惯例，以及 UI / 原型优先、后端 / 技术验证优先、双轨并行或豁免的实施顺序判断。
