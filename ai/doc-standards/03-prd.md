...
| 能力 | 最低字段 / 结构 |
|---|---|
| 文档元信息 | 输入来源、覆盖 REQ、当前状态、最后更新；「当前状态」一句话状态即可，实现历史 / Phase 演进叙事归 `09` 验收记录与 CHANGELOG，不塞元信息（需要时留历史指针） |
...
退出标准条目须含三要素（要求 / 证据形式 / 状态词），并对齐所宣称交付物档位的判据基准：见 `ai/doc-standards/stage-exit-baseline.md` §4 / §5（判据矩阵与准出条目规范）。
...
| 无实现细节倒灌 | REQ 覆盖矩阵 / 功能清单行内不出现表名、API-ID、migration 号、类名、实现算法、Sprint / PR / 版本演进史（§1 定位禁条的可审计化）；实现证据归 `09`、`docs/design/*`、CHANGELOG，需要「REQ → 实现证据」索引时另立独立索引文档并挂指针，不内联在矩阵行 | 覆盖状态列退化为实现日志；需求层与实现层混写导致双源失同步 |
