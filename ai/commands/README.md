...
- 每个命令示例标明适用 Shell；涉及路径通配时，不假设 Bash glob 在 PowerShell 或其他宿主中成立。
- 对有限且预期存在的文件，先使用精确存在性探测；PowerShell 读取路径时使用 `-LiteralPath` 等字面路径 API。可选辅助读取与 Git 状态、版本等关键事实按失败域分批，不让一个预期性未命中覆盖其他已确认结果。
- 只读路径定位失败时的边界以 `ai/rules-core.md` 为准：仅可进行一次不同的只读定位查询；构建、测试、权限、sandbox、网络、CI、远端或状态变更失败仍必须立即停止。
