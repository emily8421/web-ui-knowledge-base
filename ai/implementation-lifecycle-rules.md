...
3. 出现以下任一情况时，必须拆成 `tasks/task-00X-xxx.md`：
   ...
   - 多人 / 多 AI 会话并行；
   - 需要跨测试等级逐步验证。
4. Task 命名与内容必须能追溯回 Sprint、REQ 和 Test Case；Task 文件必须包含任务元信息、验证包、降级 / Mock 边界、完成记录和待确认项。其中「完成记录」及执行过程记录按 §4.1 结构化；非平凡任务默认适用。

### 4.1 任务卡执行记录形态（非平凡任务默认）

适用：§4 第 3 条触发条件拆出的非平凡任务卡。执行记录与代码**同一 PR 入库**，不滞留会话本地；与 `ai/session-rules.md` 续接文件分工互补——续接文件管会话间临时状态（gitignored、不入库），本节管入库可审计记录。轻量任务（单点修复、纯文档收口等）可裁剪，建议在卡内留一行说明裁剪了哪些项——形态随任务重量自适应，不搞一刀切。

1. **范围决策记录**：任务的裁决点（用户决策、边界收缩、方案取舍）在卡内留痕——决策点、结论、适用范围；作为「AI 不可自主扩展需求」（`ai/global-rules.md` §1）的执行侧证据。
2. **开工前技术事实基线**：编码前对将触碰模块的核实结论（现状行为、关键约束、已知坑），一行一事实、注明核实日期；防「文档声明 ≠ 运行时行为」类返工。与六问分工：六问核设计完备性（§5.1），事实基线记代码现状。
3. **完成记录叙事结构**（把 §4 第 4 条「完成记录」具体化，五要素）：实现记录（做了什么 + 提交 / PR 锚点）；遇到的问题（坑与当场修复方式）；偏差与决定（实现偏差 vs 设计缺口的二分处置结果，衔接 §5.1 既有口径）；验证结果（命令 + 通过计数 + 失败项）；残留（未决项与下一步）。
4. **自包含执行节（可选，多机 / 跨环境协作形态）**：需移交其他机器、环境或会话执行的步骤，写成自包含节——步骤、命令 / 脚本、证据回填位齐备，执行侧直接在卡内回填执行记录，不依赖执行侧的本地会话文件。
5. **可接续自检**：收卡自检一句——「换个一无所知的 AI，只读这张卡和仓库，能接续吗」——不能则补卡后再收。

**力度**：建议 + 默认，非强制门禁，不引入 check-template 断言（记录结构与可接续性是语义判断，无法机械化）；项目可在 `ai/project-rules.md` 细化或声明豁免；存量任务卡不回溯改写。
