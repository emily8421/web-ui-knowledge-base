...
- 设计可驱动性：按 §5.1 对该模块完成六问检查并声明结论（可驱动 / ⚠️ 已登记缺口清单）；任一问无答且未登记补齐口径时，先补设计再开工。
...
每个待实现模块（`docs/design/*` 子系统或 Sprint 内非平凡构件）进入编码前，其设计文档必须能逐项回答**可驱动性六问**：

1. **接口契约**：谁调用、传入什么、返回什么、失败如何表达。
2. **内部结构**：类、模块、状态或职责如何组织。
3. **算法逻辑**：规则、步骤、分支、优先级和降级语义。
4. **数据进出**：数据从哪里来、经过什么处理、到哪里去。
5. **错误处理**：各类失败如何检测、呈现、恢复或升级人工处理。
6. **验证基准**：用什么样例、真值、断言或检查证明实现正确。

检验方式：每问在设计中**定位到具体产物位置**（章节 / 图 ID / 表），不凭印象。任一问无答 = 不可驱动，先补设计再开工；缺口可显式登记补齐口径（如 DDL 字段级留编码期落），不构成阻塞但要在 Sprint 输入文档注明。

**占位与降级豁免边界**：依赖未就绪的占位模块可暂缓 ②③⑥ 的模块内部细节，但 ①④⑤（跨层契约面：接口契约 / 数据进出 / 错误处理）不得随占位豁免——契约面缺口会在依赖模块启动时才暴露；必交付的降级态同样过契约面检查（降级态 ≠ 免检态）。豁免必须显式登记（哪个模块、豁免哪几问、替代依据是什么），不得整体豁免。

**实现偏差 vs 设计缺口二分**：编码中发现实现与设计不符，先判定「实现偏差」（实现侧可修正，改代码对齐设计）或「设计缺口」（驱动依据不足，停任务，按 §8 / `document-lifecycle-rules` §9 变更传播规则回设计补足后再继续），不得在代码里自行发明设计。处置结果回写任务记录或 `docs/design/*` 实现偏差区（与 E6 反向同步落点约束衔接）。

**触发与力度**：Sprint 涉及非平凡模块编码前适用（E4 阶段级评估之后的单模块粒度）；Lean / 小工具项目可裁剪，须在 `ai/project-rules.md` §3 声明。本节为「建议 + 默认」而非强制门禁，不引入 check-template 断言（六问是否有答是语义判断，无法机械化）；Sprint 计划（`08`）中该模块的「输入文档」声明处可挂六问结论（可驱动 / ⚠️ 清单）作为轻量留痕。与既有规则的关系：E4 管「这批设计文档整体能否进入计划阶段」，六问管「这个模块能否开工」，层级互补；`design-doc.md` 详细设计要素是六问的检查对象（要素是否足以开工），六问不另立要素清单。
